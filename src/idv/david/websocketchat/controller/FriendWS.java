...
public class FriendWS { 
					// 使用者名稱   對應的連線 
	private static Map<String, Session> sessionsMap = new ConcurrentHashMap<>(); // JDK5 時加入 ( Concurrent 並行)
	Gson gson = new Gson(); // 老類別 HashTable 有進行 synchronized，在某一執行緒執行存取資料時鎖定範圍會較大，ConcurrentHashMap 鎖定的範圍則較小(依據 key 去鎖定)，所以效能較好
	...
	public void onMessage(Session userSession, String message) {
		ChatMessage chatMessage = gson.fromJson(message, ChatMessage.class); // 取得傳來的 JSON 字串，用 gson 去指定類別並轉換回去
